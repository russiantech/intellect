...
def uploader(file, custom_upload_path=None):
    try:
        if file and file.filename:
            file_content = file.read()
            file_hash = hashlib.md5(file_content).hexdigest()

            # Use custom upload path if provided, otherwise default to 'static/img/uploads'
            upload_folder = custom_upload_path if custom_upload_path else path.join(current_app.root_path, 'static/img/uploads')
            ...
            # Check for duplicate file by comparing hash values
            ...
            # Handle file types (images and videos)
...
# usage example
# uploader(file, custom_upload_path='/path/to/custom/folder')
...
def uploader_0(file):
    try:
        if file and file.filename:
            ...
            file_content = file.read()
            file_hash = hashlib.md5(file_content).hexdigest()
            upload_folder = path.join(current_app.root_path, 'static/img/uploads')
            output_size = (200, 300)
            existing_files = listdir(upload_folder)
